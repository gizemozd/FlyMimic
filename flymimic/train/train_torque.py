...
from flymimic.tasks.fly.mocap_tracking_torque import mocap_tracking_torque
...
def train(cfg):
    ...
    best_model_dir = logs_dir / f"best_torque_seed_{cfg.exp}_{seed}"
    ...
    train_env = Monitor(
        DMControlGymWrapper(mocap_tracking_torque(model_name=cfg.xml_name), seed=seed),
        str(logs_dir),
    )
    ...
    run_name = cfg.log_file_name or f"torque_{cfg.exp}_seed{seed}_{timestamp}"
    wandb.init(
        ...
        tags=list(wandb_cfg.tags) if wandb_cfg and hasattr(wandb_cfg, "tags") else ["torque"],
        name=run_name,
        config={
            "model": "torque",
    ...
    if cfg.load_model:
        ...
        log_file_name = f"ppo_torque_seed_{cfg.exp}_{seed}_{timestamp}"
        model = PPO(
            "MlpPolicy",
            train_env,
            learning_rate=cfg.train.learning_rate,
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix=f"ppo_mocap_agent_torque_seed_{cfg.exp}_{seed}_{timestamp}",
    )
